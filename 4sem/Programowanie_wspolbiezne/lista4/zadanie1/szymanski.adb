...
procedure Szymanski is
    ...
    procedure Await_Any (Lo, Hi : Integer; Allowed : Int_Array) is
        Found : Boolean;
    begin
        loop
            Found := False;
            for J in Lo .. Hi loop
                if In_Set (Flag_array(J), Allowed) then
                    Found := True;
                    exit;
                end if;
            end loop;
    ...
    procedure Await_Anyv2 (Id : Integer) is
        Found, Check: Boolean;
    ...
    begin
        ...
        for Step in 0 .. Nr_of_Steps/7-1 loop
            ...
            Await_Anyv2(Process.Id); -- wait for any to enter ENTRY_PROTOCOL_2
